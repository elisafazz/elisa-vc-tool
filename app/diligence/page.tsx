...
function DiligenceInner() {
  ...
  const [previousCompanies, setPreviousCompanies] = useState<PreviousCompany[]>([])
  const abortRef = useRef<AbortController | null>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)

  // Load previous companies on mount
  useEffect(() => {
    fetch('/api/research/history')
      .then(r => r.json())
      .then(data => { if (Array.isArray(data)) setPreviousCompanies(data) })
      .catch(() => {})
  }, [])
